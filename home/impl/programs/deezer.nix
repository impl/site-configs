...
{ lib, machineConfig, pkgs, ... }: with lib; mkIf machineConfig.profiles.gui.enable (let
  version = "5.30.220";
  aunetxVersion = "v${version}-2";

  electron = pkgs.electron_13.overrideAttrs (old: {
    meta = old.meta // {
  ...
  aunetxAddl = pkgs.fetchFromGitHub {
    owner = "aunetx";
    repo = "deezer-linux";
    rev = aunetxVersion;
    sparseCheckout = ''
  ...
  pkg = with pkgs; stdenv.mkDerivation rec {
    ...
    patches = filesystem.listFilesRecursive "${aunetxAddl}/patches";
    ...
    postFixup = ''
      ...
      makeWrapper "''${wrapperCmd[1]}" \
        ...
        "''${wrapperCmd[@]:2}" \
        --prefix LD_LIBRARY_PATH : ${makeLibraryPath [ udev ]} \
